"""Screenshot management module for cross-platform screen capture.

This module provides screenshot functionality across different platforms
with optimized capture methods and live detection support.
"""
...
from auto_warrior.live_capture import LiveScreenCapture

logger = logging.getLogger(__name__)


class ScreenshotManager:
    """Manages screenshot operations across different platforms with live capture support."""

    def __init__(self, debug_mode: bool = False, use_live_capture: bool = True) -> None:
        """Initialize the screenshot manager.

        Args:
            debug_mode: Whether to enable debug logging
            use_live_capture: Whether to use live capture for better performance
        """
        self.debug_mode = debug_mode
        self.platform = platform.system()
        self.use_live_capture = use_live_capture

        # Initialize live capture if enabled
        if self.use_live_capture:
            self.live_capture = LiveScreenCapture(debug_mode=debug_mode)
        else:
            self.live_capture = None

        if self.debug_mode:
            capture_method = "live capture" if self.use_live_capture else "traditional"
            logger.debug(
                f"ScreenshotManager initialized for platform: {self.platform} using {capture_method}"
            )
    ...
    def take_screenshot(self) -> Image.Image:
        ...
        """
        if self.use_live_capture:
            return self._take_screenshot_live()
        elif self.platform == "Linux":
            return self._take_screenshot_linux()
        else:
            return self._take_screenshot_default()

    def _take_screenshot_live(self) -> Image.Image:
        """Take screenshot using live capture for better performance.

        Returns:
            PIL Image object containing the screenshot

        Raises:
            ScreenshotError: If live capture fails
        """
        try:
            if self.debug_mode:
                logger.debug("Using live capture for screenshot")

            # Use PyAutoGUI directly for PIL Image
            screenshot = pyautogui.screenshot()

            if self.debug_mode:
                logger.debug(f"Live screenshot successful, size: {screenshot.size}")

            return screenshot

        except Exception as e:
            raise ScreenshotError("Live screenshot capture failed", str(e)) from e
    ...
    def _take_screenshot_linux(self) -> Image.Image:
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=SCREENSHOT_TIMEOUT)

            if result.returncode != 0:
                error_details = f"scrot stderr: {result.stderr}, stdout: {result.stdout}"
                raise ScreenshotError(
                    "scrot command failed", f"Return code: {result.returncode}, {error_details}"
                )
