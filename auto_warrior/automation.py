...
class GameAutomation:
    ...
    def _attempt_respawn_click(self, current_time: float) -> bool:
        ...
        """
        print(f"🔍 DEBUG: _attempt_respawn_click called, current attempt count: {self.state.respawn_attempt_count}")
        try:
            # Check if we need to wait between retry attempts
            if (self.state.respawn_last_attempt_time is not None and
                self.state.respawn_attempt_count > 0 and
                self.state.respawn_attempt_count < len(RESPAWN_RETRY_DELAYS)):
                ...
                if elapsed_since_last < required_delay:
                    ...
                    print(f"⏳ Waiting {remaining_delay:.1f}s before respawn attempt #{self.state.respawn_attempt_count + 1}")
                    time.sleep(1.0)
                    return True

            # Check if we've exceeded maximum attempts
            if self.state.respawn_attempt_count >= RESPAWN_MAX_ATTEMPTS:
                print(f"🔍 DEBUG: Max attempts reached ({self.state.respawn_attempt_count} >= {RESPAWN_MAX_ATTEMPTS})")
            ...
            if button_clicked:
                ...
                button_still_visible, _ = self.respawn_detector.detect_respawn_button(verify_screenshot_cv)
                
                if not button_still_visible:
                    print("🎯 Respawn successful! Button disappeared - starting post-respawn healing...")
                    ...
                    print(f"❌ Respawn attempt #{self.state.respawn_attempt_count} failed - button still visible (game may be frozen)")
            else:
                print(f"❌ Respawn attempt #{self.state.respawn_attempt_count} failed - button not found")

            # Handle failed attempt
            if self.state.respawn_attempt_count < RESPAWN_MAX_ATTEMPTS:
                delay_index = self.state.respawn_attempt_count - 1
                next_delay = RESPAWN_RETRY_DELAYS[delay_index] if delay_index < len(RESPAWN_RETRY_DELAYS) else RESPAWN_RETRY_DELAYS[-1]
            ...
            if self.state.respawn_attempt_count < RESPAWN_MAX_ATTEMPTS:
                ...
                print(f"❌ Exception occurred and maximum attempts ({RESPAWN_MAX_ATTEMPTS}) reached!")
                print(RESPAWN_FAILURE_MESSAGE)
                print("🛑 Automation stopped. Please check the game and restart when ready.")
                self.state.automation_running = False
                return False
